...
两次运行的日期推进**逐区间完全相同**（1950-10-10 → 12-25 → 1951-03-10 → 1951-05-25），而满速基准就是 0.5 日/秒 → **服务器 500 台等待车满载时仍完全跟得上实时，该特性开销低于测量分辨率（±1 日/区间 ≈ ±0.7%）**。临时造车子命令已删除并重编（`git diff --stat -- src/console_cmds.cpp` 为空，工作区只剩刻意保留的 `src/window.cpp` 崩溃诊断）。
