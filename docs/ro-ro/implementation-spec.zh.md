...
结论：差异（ON 反而快 ~6%）完全落在逐次运行噪声内 → **该特性在此规模下无可测量开销**，与 §11.1 的审计一致（每 tick 只有 O(1) 旗标判断；全车辆池扫描只在精灵路径，且总开关关闭时整段跳过）。**未覆盖**：规格里"500 台等待车 + 20 节车"的满负荷场景——游戏没有批量造车手段（控制台无建车命令、GS/AI 又设不了本特性的订单旗标），需要临时生成器或评审提供存档。
